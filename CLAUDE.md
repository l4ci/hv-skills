...
**Don't edit `.hv/` by hand — use the skill helpers.** Most of `.hv/` is now tracked (knowledge, decisions, backlog, milestones, designs, plans, spikes, per-item detail, release checklist, config). Three paths stay gitignored: `.hv/bin/` is a regenerated mirror of canonical `bin/` (overwritten on every `/hv-init`); `.hv/status.json` and `.hv/repos.json` are per-developer runtime state; `.hv/config.local.json` is per-developer config overrides deep-merged on top of `.hv/config.json` by `load_config()`. Tracked `.hv/` content is skill-owned — capture via `/hv-capture`, learn via `/hv-learn`, decide via `/hv-decide`, etc. Real code/skill changes still go in canonical sources: skill folders (`hv-*/SKILL.md`), `bin/`, `docs/`, `test/`.
