...
## Breaking: where the eight removed commands went
...
**Migration.** Run `/hv-migrate v4` in any v3 project. It rewrites `.hv/BACKLOG.md`, `.hv/plans/*`, `.hv/designs/*`, `.hv/handoff/*`, `.hv/qa/*`, and the project `CLAUDE.md` from v3-shape names to v4-shape names. Dry-run is default; `--apply` writes. Idempotent. Touched files are backed up to `.hv/migrate-backup/<timestamp>/` first. No deprecation window, no alias stubs.

## New

- **`/hv-migrate v4`** ([F19]). Version-keyed codemod for v3 → v4 project state. Rewrites the eight old command names across `.hv/` artifacts and `CLAUDE.md`; migrates `.hv/CONTEXT.md` content into the pinned `## Glossary` topic in `.hv/KNOWLEDGE.md`; sweeps orphan `bin/` helpers from older installs. Smoke section 37 adds 10 assertions covering dry-run, `--apply`, idempotent re-run, the CONTEXT-to-Glossary migration, and the orphan-bin sweep.
...
- **Helper preamble consolidation** ([F24]). `bin/hv-preamble.sh` lands as a single-line source for the `hv-self-locate` preamble convention; 62 helpers then migrate to it across three batches. Removes ~600 lines of duplicated boilerplate; every helper now sources one canonical preamble instead of carrying its own.
...
- New: `docs/announcements/v4-0.md`, the long-form release post ([F20]).
