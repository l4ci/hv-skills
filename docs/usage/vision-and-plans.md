...
hv-skills supports planning above the day-to-day backlog. `/hv-vision` frames milestones and gives your project a named destination; `/hv-plan` locks an implementation approach for a slice or item before code lands. Together they keep the orchestrator executing your written intent rather than decomposing ad-hoc from an empty context.

## /hv-vision — brainstorm milestones

`/hv-vision` is a brainstorming skill, not a templating one. It runs Socratic discovery — 2–3 questions tailored to whether you are creating a roadmap from scratch or editing an existing one — then pulls grounded findings from web research, deliberately challenges your scope and ordering, and proposes milestones with explicit dependencies. You iterate until the breakdown feels right.
...
- `.hv/milestones/M01.md`, `M02.md`, … — one detail file per milestone containing the goal, acceptance criteria, rationale, risks, and research findings for that milestone.
...
Multiple milestones can be `active` simultaneously when their dependencies allow. `/hv-next` prefers items tagged to active milestones within each priority and size band, so the active set naturally scopes work without being a hard wall — P0 bugs always jump the queue regardless of milestone, and general-backlog items without a tag are never excluded entirely.
...
For larger or higher-stakes items — size-Major or anything where you and the orchestrator need to agree on the approach before any worker dispatches — `/hv-plan` is the right step. The cost is a short focused conversation; the payoff is that six weeks from now the orchestrator is executing your written approach, not its own ad-hoc interpretation.
...
`/hv-next` prefers milestone-tagged items within each priority and size band but never excludes general-backlog items entirely. This makes the active milestone set a soft scope rather than a hard filter — you stay focused without losing visibility into the rest of the backlog.
