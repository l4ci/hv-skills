...
Term lookup via `hv-context-query` returns the union (umbrella-shared plus active sub-repo) with sub-repo entries winning on name collision. The active sub-repo is resolved from cwd via `hv-resolve-repo`.
