...
Single-repo behavior is unchanged. Umbrella-aware helpers gate on `umbrella.enabled === true` in `.hv/config.json`. Without that flag, every skill behaves exactly as before.
...
4. Accept. `/hv-init` calls `hv-umbrella-init`, which writes `.hv/repos.json` with the repos you chose and sets `umbrella.enabled: true` in `.hv/config.json`. If the umbrella is itself a git repo, `.gitignore` gains a `# ── hv umbrella ──` block listing `.claude/`, `.hv/`, and each registered sub-repo.
...
To edit the registry today, re-run `/hv-init` from the umbrella. `hv-umbrella-init` is idempotent: a second run with the same selection is a no-op; a run with new names adds them; names you omit but were previously registered are kept (with a warning). A registry editor in `/hv-config` is planned.

### Glossary terms in umbrella mode

In single-repo projects, glossary terms live as nested-bullet entries under `## Glossary` in `.hv/KNOWLEDGE.md` — written via `/hv-learn --term <name>` (helper: `hv-glossary-write`) and read via `hv-glossary-read`.

Per-sub-repo Glossary is **not yet supported** — umbrella projects share one Glossary at the umbrella root, alongside the umbrella-shared KNOWLEDGE.md learnings. Splitting Glossary per-sub-repo (umbrella-shared terms + per-sub-repo terms) is tracked as **F21** (Umbrella-aware KNOWLEDGE.md). Until F21 ships, project-wide canonical terms go in the umbrella's `.hv/KNOWLEDGE.md` Glossary.
...
Both helpers are short bash plus a small python heredoc; you can read them directly under `.hv/bin/`.
...
- **Don't create `.hv/` inside a registered sub-repo.** It masks the umbrella. `hv-resolve-umbrella` detects this and exits 2 with a `masking` message.
