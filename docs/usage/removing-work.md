...
Default mode is a dry run: the skill shows exactly what would change and asks for confirmation before writing anything. Nothing is modified until you say yes.
...
Later you discover it is a duplicate of `[F42]`. Run:
...
Counters never decrement. An ID removed today cannot be reissued to a different item tomorrow — gaps in the sequence are intentional and prevent ID collisions in git history.

GitHub issues must be closed upstream manually. `/hv-rm` has no knowledge of remote trackers.
...
Pass `--force` to override. With `--force`, the ID is stripped from the entry's items list (the entry is dropped if it becomes empty) and a warning is written to stderr. `--force` does not touch any branch or worktree — it removes only the backlog record. Branch commits survive unchanged.
...
`/hv-rm` is not a soft-delete or an undo mechanism. Once applied, the entry is gone from the active backlog. `ARCHIVE.md` keeps a historical record by default; `--scrub-archive` erases it there too.

`/hv-rm` does not close GitHub issues. Close upstream issues manually after removing a backlog entry.

`/hv-rm --force` only removes the backlog record for an active item. It does not delete the branch, revert commits, or discard work in progress. If you want to abandon a branch, use git directly after removing the backlog entry.
