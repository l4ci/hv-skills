...
`KNOWLEDGE.md` stores hard-won project knowledge: gotchas, team conventions, architectural constraints, and anything else that would waste time to rediscover. Entries are grouped under free-form topic headings. `/hv-learn` appends new learnings at the end of a session.
...
If you want to share backlog state with a team — for example, to hand off work or keep a shared list of open items — you can opt in by removing `.hv/` from `.gitignore` and committing the folder. That is a deliberate choice, not the default.
