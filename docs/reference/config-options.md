...
- `work.operatorCommand`: command used to relaunch the orchestrator inside tmux when `/hv-work` runs `work.dispatch: tmux` from a non-tmux terminal. Silent default `""`, which builds `claude --continue --model <models.orchestrator>` — `--continue` resumes the current conversation so the cycle keeps its context. Set it when your orchestrator needs a wrapper.
- `work.workerCommand`: command used to launch a worker session in its tmux window. Free text; silent default `""`, which builds `claude --model <models.worker> --permission-mode acceptEdits`. Set it when workers need a wrapper, an alias, or different permissions.
