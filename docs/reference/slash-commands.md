...
Prints the orchestrator's intended approach for an item, slice, or milestone — files it would touch, tests it would add, assumptions it is making, and known unknowns — without writing anything. Use it as a cheap gate before `/hv-work` for high-stakes items where corrections after the fact are expensive. See [next and status](../usage/next-and-status.md) for how `/hv-next` surfaces it automatically for large or risky picks.
...
Distills durable knowledge from the current session into `.hv/KNOWLEDGE.md`, grouped by topic. Captures gotchas, project conventions, constraints, debugging insights, and decisions with rationale — skips anything already obvious from reading the code. See [learning](../usage/learning.md) for the full flow.
