...
**Why hv-skills alongside Superpowers?**

[Superpowers](https://github.com/obra/superpowers) and hv-skills sit at different layers and stack rather than compete. Superpowers is a coding-execution methodology — brainstorming, plan writing, worktree isolation, TDD enforcement, code review, systematic debugging — that runs across Claude Code, Cursor, Copilot, Gemini, and Codex. hv-skills is a backlog-and-knowledge layer with durable state (`/hv-capture`, `/hv-next`, `KNOWLEDGE.md`, `DECISIONS.md`, milestones, glossary) and is deliberately single-provider on Claude Code so it can lean on `AskUserQuestion`, native subagents, and `/clear`-surviving handoff notes. Superpowers answers "how do I execute this task well?"; hv-skills answers "what do I work on next, and what did I learn last time?" Run them together: hv-skills owns `capture → next → work → ship → learn`, while Superpowers' skills (`brainstorming`, `test-driven-development`, `verification-before-completion`) activate inside `/hv-work` and `/hv-brainstorm` as execution discipline. One tension worth knowing: both define a brainstorming step, and Superpowers' system prompt pushes hard toward auto-invoking its skills, so you can hit double-prompting on the same task — `/hv-brainstorm` wins in practice because it writes to `.hv/designs/`, but expect the friction. If you want one layer, Superpowers gives cross-platform execution rigor and hv-skills gives single-project memory; if you want both, they compose.
